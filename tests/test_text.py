...
def test_labels_used_by_default(labeled_pdf):
    ...
    assert len(result) == 1
    assert result[0].page == 8  # physical position of the page labeled "1"
...
def test_label_range(labeled_pdf):
    ...
    assert [t.page for t in result] == [5, 6, 7]
...
def test_unlabeled_pdf_uses_physical_numbers(text_pdf):
    ...
    assert core.get_text(text_pdf, "2")[0].page == 2
    assert core.get_text(text_pdf, "2", physical=True)[0].page == 2
